<template>
  <div class="flex flex-col h-full overflow-hidden" style="color: rgb(var(--fg-primary));">
    <div class="px-3 py-2 ui-text-lg font-medium shrink-0" style="color: rgb(var(--fg-muted));">
      Backlinks to {{ currentFileName }}
    </div>

    <div v-if="backlinks.length === 0" class="px-3 py-4 ui-text-lg" style="color: rgb(var(--fg-muted));">
    ...
    <div v-else class="flex-1 overflow-y-auto">
      <div
        ...
        :key="idx"
        class="px-3 py-2 cursor-pointer hover:bg-[rgb(var(--bg-hover))] border-b"
      ...
      >
        <div class="flex items-center gap-2 ui-text-lg">
          ...
          <span class="ui-text-md" style="color: rgb(var(--fg-muted));">:{{ link.lineNumber }}</span>
        </div>
        <div class="mt-1 ui-text-base truncate" style="color: rgb(var(--fg-muted));">
          {{ link.context }}
        </div>
      </div>
    </div>

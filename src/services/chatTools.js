...
export const TOOL_CATEGORIES = [
  {
    ...
    subgroups: [
      ...
      {
        label: "System",
        tools: [{ name: "run_command", description: "Execute a bash command" }],
      },
    ],
  },
  {
    ...
    tools: [
      ...
      {
        name: "add_reference",
        description: "Add by DOI, arXiv, or BibTeX",
        external: "CrossRef",
      },
...
const PATH_ERROR =
  "Error: path is outside the workspace. Only files within the project folder can be accessed.";
...
const EXA_NOT_CONFIGURED =
  "This tool is not configured yet. To enable web search and URL fetching, the user can either add an Exa API key in Settings > Tools, or sign in to a Shoulders account in Settings > Account.";
...
export function getAiTools(workspace, { onFileRead } = {}) {
  ...
  const allTools = {
            ...
            try {
              ...
              if (doc.paragraphs?.length) {
                result = doc.paragraphs
                  .map((p, i) => {
                    const prefix = p.style?.startsWith("Heading")
                      ? `[${p.style}] `
                      : "";
              ...
              result =
                "[Note: ¶N: markers and [Style] tags are display-only — NOT part of the actual document text. When using anchor_text in add_comment or old_string in edit_file, use only the raw text content. Keep old_string concise — prefer short unique phrases over full paragraphs. Long strings work but are slower.]\n\n" +
                result;
    ...
    write_file: tool({
      description:
        "Create a new file or overwrite an existing one. For modifications to existing files, prefer edit_file instead.",
      inputSchema: z.object({
    ...
    edit_file: tool({
      description:
        "For text files: provide old_string (exact match) and new_string. For .docx files: use paragraph_number (from read_file output) and new_content instead — old_string/new_string do not work on DOCX.",
      inputSchema: z.object({
        path: z.string().describe("File path relative to workspace"),
        old_string: z
          .string()
          .optional()
          .describe(
            "Text files: exact string to replace (must be unique in the file)",
          ),
        new_string: z
          .string()
          .optional()
          .describe("Text files: replacement text"),
        paragraph_number: z
          .number()
          .int()
          .optional()
          .describe("DOCX only: paragraph number from read_file output (¶N)"),
        new_content: z
          .string()
          .optional()
          .describe("DOCX only: replacement text for the paragraph"),
      }),
      execute: async ({
        path,
        old_string,
        ...
        if (resolved.endsWith(".docx")) {
          ...
          if (wordBridge.isConnected(resolved)) {
            if (old_string === undefined || new_string === undefined) {
              throw new Error(
                "Word Bridge files require old_string and new_string for text edits. Use read_file first to see the document text, then use a short unique phrase as old_string.",
              );
            }
            const trackChanges = !reviews.directMode;
            const result = await wordBridge.editText(
              resolved,
              old_string,
              new_string,
              trackChanges,
          ...
          if (!sd?.activeEditor)
            return "Error: DOCX file must be open in the editor to edit.";
          ...
          try {
            sd.activeEditor.commands.replaceNodeWithHTML(
              target.node,
              `<p>${new_content}</p>`,
            );
          } finally {
          ...
          filesStore.fileContents[resolved] = extractDocumentText(
            sd.activeEditor.state,
          );
      ...
      execute: async ({ input: raw }) => {
        ...
        if (/^@\w+\s*\{/.test(trimmed)) {
          ...
          if (!entries || entries.length === 0)
            return "Failed to parse BibTeX input.";
          const results = [];
    ...
    cite_reference: tool({
      description:
        "Insert a citation [@key] at the cursor position in the active editor.",
      inputSchema: z.object({
      ...
      execute: async ({ key }) => {
        ...
        const isTexFile =
          pane.activeTab.endsWith(".tex") || pane.activeTab.endsWith(".latex");
      ...
      execute: async ({ query, num_results }) => {
        ...
        const data = await _callExa(
          "search",
          {
            ...
            summary: {
              query:
                "Return: Summary (2-sentence synthesis); Key data (statistics, dates, names); Classification (Academic Study, News Article, Blog, Marketing, etc.). Fallback for missing fields: null.",
            },
          },
          workspace,
        );
          ...
          try {
            const data = await _callExa(
              "search",
              {
                ...
                summary: {
                  query:
                    "If research article: title, authors, journal, abstract, peer-reviewed [yes/no/unclear], open source [yes/no/unclear]. If other: 2 sentence summary.",
                },
              },
              workspace,
            );
      ...
      execute: async ({ urls: rawUrls }) => {
        ...
        if (hasAccess) {
          try {
            ...
            console.warn(
              "[fetch_url] Exa failed, falling back to direct fetch:",
              e,
            );
          }
        }
        ...
        const fetchWithTimeout = (url) =>
          Promise.race([
            ...
            new Promise((_, reject) =>
              setTimeout(
                () => reject(new Error("Fetch timed out after 15s")),
                15000,
              ),
            ),
          ]);
        ...
        if (urls.length === 1) {
          try {
            return await fetchWithTimeout(urls[0]);
          } catch (e) {
            return `Error fetching ${urls[0]}: ${e.message || e}`;
          }
        }
        const results = [];
    ...
    add_comment: tool({
      ...
      inputSchema: z.object({
        ...
        proposed_edit: z
          ...
          .optional()
          .describe(
            "Optional: propose a specific text edit along with the comment",
          ),
      }),
      execute: async ({ file_path, anchor_text, text, proposed_edit }) => {
        ...
        if (wordBridge.isConnected(resolved)) {
          try {
            const result = await wordBridge.insertComment(
              resolved,
              anchor_text,
              text,
              "Shoulders AI",
            ...
            if (result.success) {
              ...
              if (result.wordCommentId)
                wordBridge.mapCommentId(shouldersId, result.wordCommentId);
    ...
    reply_to_comment: tool({
      ...
      inputSchema: z.object({
        ...
        proposed_edit: z
          ...
          .optional()
          .describe(
            "Optional: propose a specific text edit as part of your reply",
          ),
      }),
      execute: async (args) => {
          ...
          .array(
            z.object({
              ...
              url: z
                ...
                .optional()
                .describe(
                  "URL for the user to verify the source (strongly recommended — always include when available)",
                ),
              doi: z
                .string()
                .optional()
                .describe(
                  "Optional DOI — auto-adds to library on Select, derives Open link if url omitted",
                ),
            }),
          )
          .min(2)
          .max(5)
          .describe("2-5 options to present"),
      }),
    ...
    rename_file: tool({
      description:
        "Rename a file or directory. Automatically updates wiki links.",
      inputSchema: z.object({
        path: z.string().describe("Current file path relative to workspace"),
        new_name: z
          .string()
          .describe("New filename (not full path, just the name)"),
      }),
    ...
    move_file: tool({
      description:
        "Move a file to a different directory. Automatically updates wiki links.",
      inputSchema: z.object({
        path: z.string().describe("Current file path relative to workspace"),
        destination: z
          .string()
          .describe("Destination directory path relative to workspace"),
      }),
    ...
    duplicate_file: tool({
      description:
        'Create a copy of a file or directory with " (copy)" appended to the name.',
      inputSchema: z.object({
        path: z
          .string()
          .describe("File path to duplicate relative to workspace"),
      }),
      execute: async ({ path }) => {
        const resolved = _resolvePath(path, workspace);
        if (!resolved) return PATH_ERROR;
    ...
    edit_cell: tool({
      description:
        "Edit the source code of a notebook cell by index (0-based).",
      inputSchema: z.object({
        path: z.string().describe("Path to .ipynb file"),
      ...
      execute: async ({ path, index, new_source }) => {
        ...
        if (!resolved) return PATH_ERROR;
        const raw = await invoke("read_file", { path: resolved });
        const { parseNotebook, serializeNotebook } =
          await import("../utils/notebookFormat");
        const nb = parseNotebook(raw);
        ...
        if (!reviews.directMode) {
          ...
          if (existing)
            return `Cell ${index} already has a pending edit. Accept or reject it first.`;
          ...
          window.dispatchEvent(
            new CustomEvent("notebook-pending-edit", {
        ...
        const newContent = serializeNotebook(
          nb.cells,
          nb.metadata,
          nb.nbformat,
          nb.nbformat_minor,
    ...
    run_cell: tool({
      description:
        "Execute a specific code cell in a notebook and return the output. The notebook must be open in the editor with a connected kernel.",
      inputSchema: z.object({
        path: z.string().describe("Path to .ipynb file"),
        index: z.number().describe("Cell index (0-based)"),
      }),
      execute: async ({ path, index }) => {
        ...
        return new Promise((resolve, reject) => {
          const timeout = setTimeout(() => {
            ...
            reject(
              new Error(
                "Cell execution timed out (60s). Is the notebook open with a kernel connected?",
              ),
            );
          }, 60000);

          function handler(e) {
            if (e.detail?.path !== resolved || e.detail?.index !== index)
              return;
            window.removeEventListener("cell-execution-complete", handler);
          ...
          window.dispatchEvent(
            new CustomEvent("run-notebook-cell", {
    ...
    run_all_cells: tool({
      description:
        "Execute all code cells in a notebook sequentially. Returns a summary of outputs. Stops on first error.",
      inputSchema: z.object({
        path: z.string().describe("Path to .ipynb file"),
      }),
      execute: async ({ path }) => {
        ...
        return new Promise((resolve, reject) => {
          const timeout = setTimeout(() => {
            ...
            reject(
              new Error(
                "Notebook execution timed out (5 min). Is the notebook open?",
              ),
            );
          }, 300000);
          ...
          window.dispatchEvent(
            new CustomEvent("run-all-notebook-cells", {
    ...
    add_cell: tool({
      ...
      inputSchema: z.object({
        ...
        index: z
          .number()
          .optional()
          .describe("Position to insert (0 = top). Omit to append at end."),
        type: z
          .enum(["code", "markdown"])
          .optional()
          .describe("Cell type (default: code)"),
      ...
      execute: async ({ path, index, type, source }) => {
        ...
        if (!resolved) return PATH_ERROR;
        const raw = await invoke("read_file", { path: resolved });
        const { parseNotebook, serializeNotebook, generateCellId } =
          await import("../utils/notebookFormat");
        const nb = parseNotebook(raw);
        ...
        const idx =
          index != null
            ? Math.min(Math.max(0, index), nb.cells.length)
            : nb.cells.length;
        const reviews = useReviewsStore();
    ...
    read_canvas: tool({
      description:
        "Read the full graph structure of the currently open canvas (nodes, edges, content summaries). Only works when a .canvas file is active.",
      inputSchema: z.object({}),
      execute: async () => {
    ...
    add_node: tool({
      description:
        "Add a node to the canvas. Only works when a .canvas file is active.",
      inputSchema: z.object({
        type: z.enum(["text", "prompt", "file"]).describe("Node type"),
        content: z
          .string()
          .describe("Node content (text/prompt) or file path (file)"),
        ...
        connect_to: z
          ...
          .optional()
          .describe(
            "Optional node ID to connect this node to (creates edge from connect_to → new node)",
          ),
      ...
      execute: async ({ type, content, x, y, connect_to, title }) => {
        ...
        let newId;
        if (type === "text") {
          newId = canvasStore._editor.addTextNode(pos, {
            content,
            title: title || null,
          });
        } else if (type === "prompt") {
          ...
          newId = canvasStore._editor.addFileNode(pos, {
            filePath: content,
            preview: content.split("/").pop(),
          });
        } else {
    ...
    edit_node: tool({
      ...
      inputSchema: z.object({
        ...
        title: z
          ...
          .optional()
          .nullable()
          .describe("New title (null to remove)"),
      }),
    ...
    add_edge: tool({
      ...
      execute: async ({ source, target }) => {
        ...
        if (!nodes.find((n) => n.id === source))
          return `Source node not found: ${source}`;
        if (!nodes.find((n) => n.id === target))
          return `Target node not found: ${target}`;
        const { nanoid: nid } = await import("../stores/utils");
      ...
      execute: async ({ prompt, aspect_ratio }) => {
        ...
        try {
          ...
          if (result.status >= 400) {
            return `Image generation failed (${result.status}): ${data.error?.message || result.body}`;
          }
          ...
          return `Image generation error: ${e.message || e}`;
        }
      },
    }),
  };
...
export function getToolDefinitions(workspace) {
  ...
  return tools.map(([name, t]) => ({
    ...
    input_schema: t.inputSchema
      ? _zodToJsonSchema(t.inputSchema)
